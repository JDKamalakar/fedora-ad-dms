...
# Step 4: Install Dank Material Shell (DMS) - WHIPTAIL BYPASS INTEGRATED
...
if ask_yes_no "Install Dank Material Shell (DMS)?" "Y"; then
  ...
  if [ -n "$REAL_USER" ] && [ "$REAL_USER" != "root" ]; then
    sudo -u "$REAL_USER" bash -c '
      whiptail() { return 0; }
      export -f whiptail
      CI=true NONINTERACTIVE=1 bash /tmp/dms-install.sh </dev/null
    ' || true
  else
    whiptail() { return 0; }
    export -f whiptail
    CI=true NONINTERACTIVE=1 bash /tmp/dms-install.sh </dev/null || true
  fi
